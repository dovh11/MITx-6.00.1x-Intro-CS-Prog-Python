...
def getWordScore(word, n):
    ...
    for s in word:
        points += SCRABBLE_LETTER_VALUES[s]
...
def isValidWord(word, hand, wordList):
    ...
    update_hand = updateHand(hand, word)
    for num in update_hand.values():
        if num < 0:
            result = False
    ...
    while calculateHandlen(hand) >= 1:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print("\"" + userInput + "\"" + " earned " + str(getWordScore(userInput, n)) + ". Total: " + str(totalScore) + " points.")
                print()
                # Update the hand 
...
def playGame(wordList):
    ...
    while True:
        ...
        if userInput not in validInput:
            ...
            if userInput == 'n':
                handHistory = dealHand(HAND_SIZE)
                playHand(handHistory, wordList, HAND_SIZE)
                print()
            elif userInput == 'r':
                try:
                    handHistory
